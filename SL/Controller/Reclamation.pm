...
sub action_delete {
  ...
  flash_later('info', $::locale->text('The reclamation has been deleted'));
...
sub action_save {
  ...
  flash_later('info', $::locale->text('The reclamation has been saved'));
...
sub action_save_as_new {
  ...
  $new_attrs{rec_number} = (trim($reclamation->rec_number) eq $saved_reclamation->rec_number)
                         ? ''
                         : trim($reclamation->rec_number);
...
sub action_print {
  ...
  my $form   = Form->new;
  $form->{rec_number} = $self->reclamation->rec_number;
  $form->{type}       = $self->type;
  $form->{format}     = $format;
...
sub action_reorder_items {
  ...
  my %sort_keys = (
    ...
    rec_reason   => sub { $_[0]->rec_reason->name },
...
sub js_reset_reclamation_and_item_ids_after_save {
  ...
  $self->js
    ...
    ->val('#reclamation_rec_number', $self->reclamation->rec_number);
...
sub init_models {
  ...
  SL::Controller::Helper::GetModels->new(
    ...
    sorted => {
      ...
      id                      => t8('ID'),
      rec_number              => t8('Reclamation Number'),
      ...
      delivery_term_id        => t8('Delivery Term'),
      payment_id              => t8('Payment Term'),
    ...
    query => [
      ...
      (employee_id => SL::DB::Manager::Employee->current->id) x ($self->reclamation->is_sales  && !$::auth->assert('sales_all_edit', 1)), # TODO(Tamino): is this right?
...
sub workflow_save_and_sales_or_purchase_reclamation {
  ...
  #TODO(Tamino): is direct delivery needed?
...
sub pre_render {
  ...
  if ($self->reclamation->rec_number && $::instance_conf->get_webdav) {
    my $webdav = SL::Webdav->new(
      type     => $self->type,
      number   => $self->reclamation->rec_number,
    );
...
sub prepare_report {
  ...
  $column_defs{$_}->{text} ||= $::locale->text( $self->models->get_sort_spec->{$_}->{title} || $_ ) for keys %column_defs;
    ...
    $bar->add(
      ...
      combobox => [
        ...
        action => [
          ...
          call      => [ 'kivi.Reclamation.purchase_reclamation_check_for_direct_delivery' ], # TODO(Tamino): is this needed? ohterwise:
          #call      => [
          #  'kivi.Reclamation.save', 'save_and_purchase_reclamation',
          #  $::instance_conf->get_reclamation_warn_duplicate_parts,
          #  $::instance_conf->get_reclamation_warn_no_reqdate,
          #],
...
sub get_title_for {
  ...
  # for locales:
  # $::locale->text("Add Sales Reclamation");
  # $::locale->text("Add Purchase Reclamation");
  # $::locale->text("Edit Sales Reclamation");
  # $::locale->text("Edit Purchase Reclamation");

  $action = ucfirst(lc($action));
  return $self->type eq sales_reclamation_type()    ? $::locale->text("$action Sales Reclamation")
       : $self->type eq purchase_reclamation_type() ? $::locale->text("$action Purchase Reclamation")
       : '';
}
...
sub save_history {
  ...
  SL::DB::History->new(
    ...
    snumbers    => 'rec_number_' . $self->reclamation->rec_number,
    addition    => $addition,
  )->save;
}
...
sub store_pdf_to_webdav_and_filemanagement {
  ...
  if ($reclamation->rec_number && $::instance_conf->get_webdav_documents) {
    my $webdav = SL::Webdav->new(
      type     => $reclamation->type,
      number   => $reclamation->rec_number,
    );
...
Rereclamationing item rows with drag and drop is possible. Sorting item rows is
possible (by partnumber, description, rec_reason, shipped_qty, qty, sellprice
