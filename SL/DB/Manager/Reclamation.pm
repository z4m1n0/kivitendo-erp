...
__PACKAGE__->add_filter_specs(
  ...
  #all => sub {
  #  my ($key, $value, $prefix) = @_;
  #  return or => [ map { $prefix . $_ => $value } qw(rec_number customer.name vendor.name transaction_description) ]
  #}
);
  ...
  return (
    ...
    columns                   => {
      ...
      map { ( $_ => "lower(reclamations.$_)" ) }
        qw(rec_number cv_record_number shippingpoint shipvia notes intnotes
