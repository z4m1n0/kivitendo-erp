...
__PACKAGE__->configure_acts_as_list(group_by => [qw(rec_id)]);
...
sub new_from {
  ...
  my @custom_variables = map { _clone_cvar_for_rec_item($_) } @{ $source->custom_variables };
  ...
  if (ref($source) eq 'SL::DB::ReclamationItem') {
    map { $item_args{$_} = $source->$_ } qw(
      ...
      pricegroup_id project_id qty rec_reason_description_ext
      rec_reason_description_int rec_reason_id reqdate sellprice serialnumber
      unit
    );
  ...
  my $rec_item = $class->new(%item_args);

  if ( $source->record->is_sales() && ($parent_type =~ m{sales}) ) {
    $rec_item->sellprice($source->lastcost);
    $rec_item->discount(0);
  }
  if ( !$source->record->is_sales() && ($parent_type =~ m{purchase}) ) {
    $rec_item->lastcost($source->sellprice);
  }

  $rec_item->assign_attributes(%{ $params{attributes} }) if $params{attributes};

  unless ($params{no_linked_records}) {
    $rec_item->{"converted_from_record_item_type_ref"} = ref($source);
    $rec_item->{"converted_from_record_item_id"} = $source->id;
  }

  return $rec_item;
}

sub _clone_cvar_for_rec_item {
  my ($cvar) = @_;
...
sub record_id { goto &rec_id }
sub trans_id { goto &rec_id }
