...
__PACKAGE__->meta->add_relationship(
  ...
  reclamation_items => {
    ...
    column_map   => { id => 'rec_id' },
    manager_args => {
      with_objects => [ 'part', 'rec_reason' ]
    }
  },
  custom_shipto            => {
    type                   => 'one to one',
...
sub _before_save_set_rec_number {
  my ($self) = @_;
...
sub _before_save_set_custom_shipto_module {
  ...
  $self->custom_shipto->module('RC') if $self->custom_shipto;
...
sub record_number { goto &rec_number; }
...
sub displayable_name {
  join ' ', grep $_, map $_[0]->$_, qw(displayable_type rec_number);
};
