...
CREATE TABLE reclamation_reasons (
  ...
  valid_for                REC_REASONS_VALID_FOR       NOT NULL
);
...
ALTER TABLE defaults ADD COLUMN p_rec_number TEXT NOT NULL DEFAULT 0;
ALTER TABLE defaults ADD COLUMN s_rec_number TEXT NOT NULL DEFAULT 0;

CREATE TABLE reclamation_items (
--base
  id                         SERIAL                      PRIMARY KEY,
  rec_id                     INTEGER                     NOT NULL REFERENCES reclamations(id) ON DELETE CASCADE,
  rec_reason_id              INTEGER                     NOT NULL REFERENCES reclamation_reasons(id),
  rec_reason_description_ext TEXT,
  rec_reason_description_int TEXT,
