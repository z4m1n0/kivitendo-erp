namespace('kivi.Reclamation', function(ns) {
  ...
  ns.check_valid_rec_reasons = function() {
    var positions_with_empty = [];

    $('.row_entry').each(function(idx, elt) {
      if ($(elt).find('[name="reclamation.reclamation_items[].rec_reason_id"]').val() == '') {
  ...
  ns.save = function(action, warn_on_duplicates, warn_on_reqdate) {
    ...
    if (!ns.check_valid_rec_reasons())                     return;
  ...
  ns.follow_up_window = function() {
    var id   = $('#id').val();
    var type = $('#type').val();
    var number_info = $('#reclamation_rec_number').val();
