...
% laufende Kopfzeile:
\ourhead{\kundennummer}{$( KiviLatex.filter(reclamation.customervendor.number) )$}{\auftragsbestaetigung}{$( KiviLatex.filter(reclamation.rec_number) )$}{$( reclamation.transdate_as_date )$}
...
\setkomavar{title}{
  ...
  \nr~$( KiviLatex.filter(reclamation.rec_number) )$%
}
...
\begin{PricingTabular*}
  % eigentliche Tabelle
  \FakeTable{
  $( FOREACH rec_item = reclamation.items_sorted )$%
    $( rec_item.position )$ &%
    $( rec_item.part.partnumber )$ &%
    \textbf{ $( KiviLatex.filter(rec_item.description) )$ }%
    $( IF (rec_item.longdescription) )$\ExtraDescription{$( KiviLatex.filter(rec_item.longdescription) )$}$( END )$%
    $( IF (rec_item.reqdate) )$\ExtraDescription{\lieferdatum: $( rec_item.reqdate_as_date )$}$( END )$%
    $( IF (rec_item.serialnumber) )$\ExtraDescription{\seriennummer: $( KiviLatex.filter(rec_item.serialnumber) )$}$( END )$%
    $( IF (rec_item.projectnumber) )$\ExtraDescription{\projektnummer: $( KiviLatex.filter(rec_item.projectnumber) )$}$( END )$%
    &%
    $( rec_item.qty_as_number )$ $( rec_item.unit )$ &%
    $( rec_item.sellprice_as_number )$&%
    % TODO(Tamino): discount in '%'
    \Ifstr{$( p_discount )$}{0}{}{\sffamily\scriptsize{(-$( p_discount )$\,\%)}}%
    % TODO(Tamino): linetotal
    $( LxERP.format_amount(rec_item.linetotal, 2, 0) )$\tabularnewline
  $( END )$%
  }%
  \begin{PricingTotal}%
